...
export default function DateTimePicker({ value, onChange, placeholder = "Pick a date & time", className }) {
  const init = React.useMemo(() => initFromISO(value), [])  // only on mount
  ...
  function handleHour(h) {
    setHour(h)
    notify(date, h, minute, ampm)
  }
  function handleMinute(m) {
    setMinute(m)
    notify(date, hour, m, ampm)
  }
  function handleAmpm(ap) {
    setAmpm(ap)
    notify(date, hour, minute, ap)
  }
    ...
    <div className={cn("flex flex-col gap-3", className)}>
      ...
      <div className="flex items-center gap-2">
        ...
        {date && (
          <button
            type="button"
            onClick={() => { setDate(undefined); onChange?.(null) }}
